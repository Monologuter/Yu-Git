...
public struct CommitComposer: Sendable {
    ...
    public struct Proposal: Sendable, Equatable {
        ...
        public init(commits: [ComposedCommit], unassignedHunkIDs: [String]) {
    ...
    static let systemPrompt = """
        ...
        判断标准是**意图**而不是文件：同一个文件里的两处改动如果做的是两件事，\
        就该分到两个提交；分散在五个文件里的改动如果服务于同一个目的，就该合成一个。

        分组原则：
        - 一个提交只做一件事，能用一句话说清
        - 重构与功能改动分开；格式化、依赖升级这类噪音单独成组
        - 拿不准就少分几组——分得过细比分得过粗更烦人，用户还得手动合回去
        - 有依赖关系的改动放同一组，或者让被依赖的那组排在前面
    ...
    static func parse(_ text: String, hunks: [ComposableHunk]) throws -> Proposal {
        ...
        return Proposal(commits: commits, unassignedHunkIDs: unassigned)
    }
