...
1. **零视觉辨识度** → 引入品牌色「青」`#017272`，出现在系统色管不到的地方：当前分支标记、
   分支图主线、AI 入口、空状态、进度点、品牌字。纯蓝仍然留给系统强调色（选中、焦点），
   因为默认主题要跟随系统。
2. **commit graph 太弱** → 线宽 1.8 → 2.5，HEAD 所在轨道 3.5；节点 4.5 环 + 中心挖空；
   合并提交改成实心点、HEAD 加一圈品牌青细环（形状区分，不只靠颜色）；
   8 轨道色板重做成等距色相 + 两档明度。
3. **列表缺少节奏** → 新增日期分组行（24pt，粘顶），一行高度换来整列的落点，不动信息密度。
4. **空状态平淡** → `EmptyState` 组件：品牌青圆底图标 + 标题说清情况 + 一句下一步。

**没做的**：墨 / 昼 / 谱 三套主题。简报要求先只做「默认」，风格确认后再扩展 ——
现在落了默认主题的浅色与深色两组（共 8 组中的 2 组）。
...
**颜色**。三条占用规则先于一切：红绿属于 diff；纯蓝属于系统强调色；青属于品牌。
...
**这份设计系统里是 Lucide 替身**（`assets/icons/*.svg`，63 个，MIT）。
原因很实际：SF Symbols 不能随设计系统分发，而网页预览需要能看见图标。
Lucide 是最近的一档 —— 同为描边、圆头端点、2px 标称线宽、24 网格。
**这是替换品，验收时以 macOS 上的真机截图为准。** 映射表在 `components/core/Icon.prompt.md`。

用法规则：
- 尺寸只用 11（配 11pt 文字）/ 14（配正文与列表行）/ 16（工具栏）三档，外加空状态里的 22。
- 颜色一律 `currentColor`，跟着文字走。三个例外：AI 入口的 `sparkles` 永远品牌青，
...
| `assets/icons/` | 63 个 Lucide SVG（SF Symbols 替身） |
| `github.md` | 仓库关联与同步记录 |
...
2. **图标**：Lucide 替 SF Symbols，见上。
3. **没有 logo**：仓库里不存在任何图形标志，这份设计系统**没有替它造一个**。
   所有需要「标志」的地方用字组：青底白「驭」（96×96，圆角 22）或「驭Git」文字。
   如果之后有了真的 logo，替换点只有两处：`ui_kits/first-run/Welcome.jsx` 与 `thumbnail.html`。
4. **只落了 2 / 8 组色值**：默认主题的浅色与深色。墨 / 昼 / 谱 等确认。
5. **两处色相值得复核**：碧 `#00A86E`（160°）与 diff 绿（152°）色相相近 ——
   靠「diff 绿只做整行底与单字符标记、轨道色只做 2.5pt 线条」区分职责，
   如果你觉得仍然容易误读，可以把它换成更偏青的 172°（代价是和青轨道更接近）。
