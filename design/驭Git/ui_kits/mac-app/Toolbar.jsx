...
function Toolbar({ repo, branch, transferring, onFetch, onPull, onPush, onPalette, onTimeline, timelineOpen, onToggleSidebar, sidebarOpen }) {
  return (
    <div style={{
      ...
      <TrafficLights />
      <ToolbarButton icon="sidebar.left" title={sidebarOpen ? '隐藏侧栏' : '显示侧栏'} onClick={onToggleSidebar} active={sidebarOpen} />
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingLeft: 'var(--space-regular)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.15 }}>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, font: 'var(--type-secondary)', color: 'var(--ink-2)' }}>
            <Icon name="arrow.triangle.branch" size={10} color="var(--brand)" />{branch}
          </span>
        </div>
      </div>
      <span style={{ flex: 1 }} />
      {transferring ? <TransferIndicator label="对象 1240 / 3877" /> : null}
      <ToolbarButton icon="arrow.down.circle" title="从远程拉取引用与对象，不改动工作区" disabled={transferring} onClick={onFetch} />
      ...
      <ToolbarButton icon="clock.arrow.circlepath" title="时间线" active={timelineOpen} onClick={onTimeline} />
      <ToolbarButton icon="ellipsis.circle" title="命令面板、搜索、时间线、刷新" onClick={onPalette} />
    </div>
  );
}
